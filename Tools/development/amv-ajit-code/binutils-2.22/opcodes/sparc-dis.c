...
/*   char *amv_tracefile = "/dev/null"; */
/* #define AMV_STR_LEN   64 */
...
/* #define STRVAL            3 */
/* #define REGVAL            4 */
/* #define print_field(val,valtype,fmt)      for (amv_i = 0; amv_i < AMV_STR_LEN; amv_str[amv_i++] = 0); \ */
/*                                             sprintf (amv_str,                                         \ */
/*                                                      (valtype == INTVAL)    ? "%d"   :                \ */
/*                                                      (valtype == HEXINTVAL) ? "%X"   :                \ */
/*                                                      (valtype == REGVAL)    ? "%%%s" :                \ */
/*                                                      (valtype == STRVAL)    ? "%s"   : "\"%s\"",      \ */
/*                                                      val);                                            \ */
/*                                            fprintf (amv_trace, fmt, amv_str, amv_sep_char); */

/*   amv_trace = fopen (amv_tracefile, "w+"); */
/*   if (amv_trace == NULL) { */
/*     /\* perror ("Error"); *\/ */
/*     fprintf (stderr, "File %s open error with error number %d.\n", amv_tracefile, errno); */
/*     (*info->fprintf_func) (stream,  "### ;;; AMV: FAILED to open file %s ...\n", amv_tracefile); */
/*     return -1;                  /\* arbit number to signal failure! *\/ */
/*   } */


/*   print_field("Name",        STRVAL, "%10s%c"); */
/*   print_field("Match",       STRVAL, "%10s%c"); */
/*   print_field("Lose",        STRVAL, "%10s%c"); */
/*   print_field("Args",        STRVAL, "%10s%c"); */
/*   print_field("Flags",       STRVAL, "%10s%c"); */
/*   print_field("Arch",        STRVAL, "%10s%c"); */
/*   print_field ("Supported?", STRVAL, "%10s%c"); */
/*   print_field ("Match?",     STRVAL, "%10s%c"); */
/*   print_field ("Lose?",      STRVAL, "%10s%c"); */
/*   print_field ("RS1",        STRVAL, "%10s%c"); */
/*   print_field ("RS2",        STRVAL, "%10s%c"); */
/*   print_field ("RD",         STRVAL, "%10s%c"); */
/*   print_field ("rs1|imm?",   STRVAL, "%10s%c"); */
/*   print_field ("rs1+imm?",   STRVAL, "%10s%c"); */
/*   print_field ("rs1==rd?",   STRVAL, "%10s%c"); */
/*   print_field ("rs2==rd?",   STRVAL, "%10s%c"); */
  ...
  for (op = opcode_hash_table[HASH_INSN (insn)]; op; op = op->next)
    {
      ...
      /* /\* if (amv_trace != NULL) { *\/ */
      /*   print_field (opcode->name,         STRVAL,    "%10s%c"); */
      /*   print_field (opcode->match,        HEXINTVAL, "0x%08X%c"); */
      /*   print_field (opcode->lose,         HEXINTVAL, "0x%08X%c"); */
      /*   print_field (opcode->args,         STRVAL,    "%10s%c"); */
      /*   print_field (opcode->flags,        HEXINTVAL, "0x%08X%c"); */
      /*   print_field (opcode->architecture, HEXINTVAL, "0x%08X%c"); */
      /*   /\* fprintf (amv_trace, "\n"); *\/ */
      /*   /\* fflush (amv_trace); *\/ */
      /* 	/\* fclose (amv_trace); *\/ */
      /* /\* } *\/ */
      ...
      /* /\* amv_trace = fopen (amv_tracefile, "a"); *\/ */
      /* /\* if (amv_trace != NULL) { *\/ */
      /*   print_field ((!(opcode->architecture & current_arch_mask)) ? "No" :"Yes", STRVAL, "%-10s%c"); */
      /*   /\* fflush (amv_trace); *\/ */
      /* 	/\* fclose (amv_trace); *\/ */
      /* /\* } *\/ */
      ...
      /* /\* amv_trace = fopen (amv_tracefile, "a"); *\/ */
      /* /\* if (amv_trace != NULL) { *\/ */
      /*   print_field (((opcode->match & insn) == opcode->match) ? "Yes" :"No", STRVAL, "%10s%c"); */
      /*   print_field (((opcode->lose  & insn) == 0)             ? "Yes" :"No", STRVAL, "%10s%c"); */
      /*   /\* fflush (amv_trace); *\/ */
      /* 	/\* fclose (amv_trace); *\/ */
      /* /\* } *\/ */
      if ((opcode->match & insn) == opcode->match
	  ...
	  /* /\* amv_trace = fopen (amv_tracefile, "a"); *\/ */
	  /* /\* if (amv_trace != NULL) { /\\* AMV *\\/ *\/ */
	  /*   print_field (reg_names [X_RS1(insn)],  REGVAL, "%10s%c"); */
	  /*   print_field (reg_names [X_RS2(insn)],  REGVAL, "%10s%c"); */
	  /*   print_field (reg_names [X_RD(insn)],   REGVAL, "%10s%c"); */
	  /*   print_field ((opcode->match == 0x80102000) ? "Yes" :"No", STRVAL, "%10s%c"); */
	  /*   print_field ((opcode->match == 0x80002000) ? "Yes" :"No", STRVAL, "%10s%c"); */
	  /*   print_field ((X_RS1 (insn) == X_RD(insn))  ? "Yes" :"No", STRVAL, "%10s%c"); */
	  /*   print_field ((X_RS2 (insn) == X_RD(insn))  ? "Yes" :"No", STRVAL, "%10s%c"); */
	  /*   /\* fflush (amv_trace); *\/ */
	  /*   /\* fclose (amv_trace); *\/ */
	  /* /\* } *\/ */
