...
def buildUniquifyMaps (uniquification_file):
	  ...
	  add_link = []
          if split_line[0] == "prefix_rename":
	        ...
	        add_link.append(split_line[1] + "_" + split_line[3] + "_")
                if ret_prefix_rename_map.has_key(sys_prefix):
		...
		add_link.append(split_line[1] + "_" + split_line[3] + "_ " + split_line[4] + " " + split_line[5])
                if ret_rpipe_rename_map.has_key(sys_prefix):
		...
		add_link.append(split_line[1] + "_" + split_line[3] + "_ " + split_line[4] + " " + split_line[5])
                if ret_wpipe_rename_map.has_key(sys_prefix):
    ...
    return ret_prefix_rename_map, ret_rpipe_rename_map, ret_wpipe_rename_map
                 
def uniquifyAa2cDirectory (aa2c_dir, prefix_rename_map, rpipe_rename_map, wpipe_rename_map):
    ...
    for cfile in os.listdir(aa2c_dir):
       ...
       if (cfile == (sys_prefix + "aa_c_model.h")):
          ...
          suffix = "aa_c_model.c"
       if suffix != "": 
           createRenamedFiles (sys_prefix, suffix,  aa2c_dir, cfile, prefix_rename_map, rpipe_rename_map, wpipe_rename_map)

    return 0

           
def createRenamedFiles (sys_prefix, suffix, aa2c_dir, cfile, prefix_rename_map, rpipe_rename_map, wpipe_rename_map):
...
def main():
	...
	for aafile in args:
	   ...
	   for aafile in args:
           ...
           if(hsysuqf_status):
              logError("hierSysPartition (uniquify) failed in " + root)
              return 1

           if uniquify_flag:
              uniquification_file = "__UFQ.TXT"
              prefix_rename_map,  rpipe_rename_map, wpipe_rename_map  = buildUniquifyMaps (uniquification_file)
    elif (hiersys2c_flag):
        ...
        for name in dirs:
           ...
           if(compile_aa2c_files and (name == "aa2c")):
              ...
              if uniquify_flag:
                 ...
                 uniquifyAa2cDirectory (full_name, prefix_rename_map, rpipe_rename_map, wpipe_rename_map)
