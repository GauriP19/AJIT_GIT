...
def buildExecutable(work_area, link_sections, obj_files, linker_script_file, elf_file, hex_dump_file,variable_dump_file,memmap_file, objdump_file):
...
def parseOptions(opts):
    ...
    return help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings , debug_mode, opt_level, link_sections, compile_options
...
def main():
    ...
    arg_list = sys.argv[1:]
    opts,args = getopt.getopt(arg_list,'W:L:C:c:S:s:I:D:E:H:V:M:O:hgo:T:F:')
    help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings, debug_mode, opt_level,link_sections,compiler_options  = parseOptions(opts)
    ...
    if(elf_file != None):
       ret_status = buildExecutable(work_area, link_sections, obj_files, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file)
       if(ret_status == 1):
