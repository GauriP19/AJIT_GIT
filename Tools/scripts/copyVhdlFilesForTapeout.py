...
def main():
    ...
    output_directory = "./"
    root_directory = "./"
    opts,args = getopt.getopt(arg_list,'r:o:')
    app_name = ""
    c_include_dirs = []
    cc_flags =  " "
    ...
    aieee_cpy_cmd = "deleteAsserts.py -i " + ahir_release + "/vhdl/aHiR_ieee_proposed.vhdl -o " + output_directory  + "/aHiR_ieee_proposed.vhdl"
    execSysCmd (aieee_cpy_cmd)

    sed_cmd = "sed_synopsys_sync_set_reset.sh " + ahir_release + "/vhdl/ahirForAsic.vhdl " + output_directory + "/.ahirASIC.txt"
    execSysCmd (sed_cmd)

    a_cpy_cmd = "deleteAsserts.py -i " + output_directory + "/.ahirASIC.txt -o " + output_directory + "/ahirASIC.vhdl"
    execSysCmd (a_cpy_cmd)
    ...
    cio_cmd =  "cp -f " + root_directory + "/chip_with_io.vhdl " + output_directory + "/"
    execSysCmd (cio_cmd) 
